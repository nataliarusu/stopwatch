...
const timerOn=()=>{
    interval=setInterval(()=>{
        ...
        if(timer.cs_seconds===100){
            timer.cs_seconds=0;
            timer.seconds+=1;           
            secondsEl.innerHTML=timer.seconds<10 ?`0${timer.seconds}`:`${timer.seconds}`;
        }
        if(timer.seconds===60){
            timer.seconds=0;
            timer.minutes+=1;
            minutesEl.innerHTML=timer.minutes<10 ?`0${timer.minutes}`:`0${timer.minutes}`;
        }
        if(timer.minutes===60){
            timer.minutes=0;
            timer.hours+=1;
        ...
        csSecondsEl.innerHTML=timer.cs_seconds<10?`0${timer.cs_seconds}`:`${timer.cs_seconds}`;
    },10);
}
const timerOff=()=>{
